import logo from "./logo.png";
import GoogleSVG from "./svg/google.svg";
import InboxSVG from "./svg/inbox.svg";
import EducationSVG from "./svg/education.svg";
import SearchSVG from "./svg/search.svg";
import SuitcaseSVG from "./svg/suitcase.svg";
import VerifiedSVG from "./verified.svg";
import WorkSVG from "./svg/work.svg";
import TwitterSVG from "./svg/socials/twitter.svg";
import GithubSVG from "./svg/socials/github.svg";
import InstagramSVG from "./svg/socials/instagram.svg";
import YoutubeSVG from "./svg/socials/youtube.svg";
import LinkedinSVG from "./svg/socials/linkedin.svg";
import FuelerSVG from "./svg/socials/fueler.svg";
import ExternalLinkSVG from "./svg/external_link.svg";
import MedalSVG from "./svg/medal.svg";
import MessageSVG from "./svg/message1.svg";
import FilterSVG from "./svg/filter.svg";
import CameraSVG from "./svg/camera.svg";
import PlusCircleSVG from "./svg/plus_circle.svg";
import LinkTilted from "./svg/link_tilted.svg";
import UsersSVG from "./svg/users.svg";
import EditSVG from "./svg/edit1.svg";
import DeleteSVG from "./svg/delete.svg";
import CheckRoundedFilledSVG from "./svg/checkFill.svg";
import DashedCircleSVG from "./svg/dashed-circle.svg";
import ChevronDownSVG from "./svg/chevron-down.svg";
import DribbleSVG from "./svg/socials/dribble.svg";
import GlobeSVG from "./svg/socials/globe.svg";
import MediumSVG from "./svg/socials/medium.svg";
import ArrowSVG from "./svg/arrow.svg";
import ArrowDiagonalSVG from "./svg/arrowDiagonal.svg";
import CommenetSVG from "./svg/comment.svg";
import ShareSVG from "./svg/shareIcon.svg";
import SaveSVG from "./svg/save.svg";
// import inbox from "./message.png";

import MyPostSVG from "./svg/myPost.svg";

import deleteIcon from "./delete.png";
import locationIcon from "./location.png";
import pinIcon from "./pin.png";

import PictureSVG from "./svg/picture.svg";
import JobSVG from "./svg/job.svg";
import SendSVG from "./svg/send.svg";
import FeedSVG from "./svg/feed.svg";
import GroupSVG from "./svg/group.svg";
import NotificationSVG from "./svg/notification.svg";
import f1 from "./features/f1.png";
import f2 from "./features/f2.png";
import f3 from "./features/f3.png";
import f4 from "./features/f4.png";
import Chirp3SVG from "./svg/landingPage/chirp3.svg";
import Chirp4SVG from "./svg/landingPage/chirp4.svg";

import BookmarkSVG from "./svg/bookmark.svg";
import DiscoverSVG from "./svg/discover.svg";
import ThreeDotsSVG from "./svg/threeDots.svg";
import ReportSVG from "./svg/report.svg";
import AvatarSVG from "./svg/avatar.svg";
import TimeSVG from "./svg/time.svg";

import logoutIcon from "./icons/logout.png";
import viewIcon from "./icons/view.png";
import MoonIcon from "./icons/moon.png";
import SunIcon from "./icons/sun.png";

export {
  MoonIcon,
  SunIcon,
  viewIcon,
  logoutIcon,
  logo,
  TimeSVG,
  SaveSVG,
  BookmarkSVG,
  DiscoverSVG,
  ThreeDotsSVG,
  ReportSVG,
  Chirp4SVG,
  Chirp3SVG,
  f1,
  f2,
  f3,
  f4,
  ArrowSVG,
  PictureSVG,
  JobSVG,
  SendSVG,
  FeedSVG,
  GroupSVG,
  pinIcon,
  locationIcon,
  deleteIcon,
  MyPostSVG,
  CommenetSVG,
  ShareSVG,
  GoogleSVG,
  InboxSVG,
  EducationSVG,
  SearchSVG,
  SuitcaseSVG,
  VerifiedSVG,
  WorkSVG,
  TwitterSVG,
  GithubSVG,
  InstagramSVG,
  YoutubeSVG,
  LinkedinSVG,
  FuelerSVG,
  ExternalLinkSVG,
  MedalSVG,
  MessageSVG,
  FilterSVG,
  CameraSVG,
  PlusCircleSVG,
  LinkTilted,
  UsersSVG,
  EditSVG,
  DeleteSVG,
  CheckRoundedFilledSVG,
  DashedCircleSVG,
  ChevronDownSVG,
  DribbleSVG,
  GlobeSVG,
  MediumSVG,
  ArrowDiagonalSVG,
  NotificationSVG,
  AvatarSVG,
};
